...
class RideOperations:
    ...
    def ride_map_process(self, result_data, location):
        try:
            ...
            result_data = result_data[result_data[location] != ""]
            location_data["long"] = result_data[location].apply(lambda x: x["longitude"] \
                if isinstance(x,dict) and 'longitude' in x.keys() else '')
            location_data["lat"] = result_data[location].apply(lambda x: x["latitude"] \
                if isinstance(x,dict) and 'latitude' in x.keys() else '')
