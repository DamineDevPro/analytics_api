...
from analytics.settings import  UTC, db, CURRENCY_API, BASE_CURRENCY, DEVICE_SUPPORT, BOOKING_STATUS
...
class Zones(APIView):
    def get(self, request):
        try:
            ...
            print("city_id type--->", type(city_id))
            if not city_id or city_id == "null":
                response = {"message": "No data found"}
                return JsonResponse(response, safe=False, status=status.HTTP_204_NO_CONTENT)
            query = {"cityId": {"$in" : [ObjectId(city_id) ,city_id]}}
            projection = {"_id": 1, "title": 1}
            result_data = pd.DataFrame(db["areaZones_rides"].find(query, projection))
            if not result_data.shape[0]:
                response = {"message": "No Data found", "data": []}
                return JsonResponse(response, safe=False, status=status.HTTP_204_NO_CONTENT)
            result_data["id"] = result_data["_id"].apply(lambda x: str(x))
            result_data.rename(columns={"title": "name"}, inplace=True)
            result_data = result_data.drop("_id", axis=1, errors="ignore")
