...
class TruckerOperations:
    ...
    def trucker_status(self, order, time_zone, start_timestamp, end_timestamp, group_by):
        try:
            ...
            order['bookingStatus'] = order["status"].apply(lambda x: int(x['status']) if x else 0)
            order['bookingStatus'] = order["bookingStatus"].replace({9: "cancelled"})
            order['bookingDateTimestamp'] = order['bookingDateTimestamp'].apply(
                lambda x: datetime(day=x.day, month=x.month, year=x.year, hour=x.hour))
    ...
    def trucker_payment(self, order, time_zone, start_timestamp, end_timestamp, group_by):
        # try:
        order["createdDate"] = pd.to_datetime(order["createdDate"], unit='s')
        order['createdDate'] = order['createdDate'].apply(
            lambda x: x.replace(tzinfo=UTC).astimezone(time_zone))
        order["dateTime"] = order.createdDate.apply(
            lambda x: datetime(day=x.day, month=x.month, year=x.year, hour=x.hour))
        # order["count"] = 1
        order = order.drop("estimate_fare", axis=1, errors="ignore")
        try:
            order['typeId'] = order['typeId'].apply(
                lambda x: x.oid if not isinstance(x, int) else str(x))
        except:
            order['typeId'] = order['typeId'].astype(str)
        ...
        order['bookingDateTimestamp'] = order['createdDate'].apply(
            lambda x: datetime(day=x.day, month=x.month, year=x.year, hour=x.hour))
