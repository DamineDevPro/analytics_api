...
class TruckerOperations:
    ...
    def descriptive_stats(self, previous_sum, current_sum, previous_start_date, previous_end_date, today_sum):
        ...
        try:
            ...
            return TruckerResponses.get_status_200(response=response)
        except Exception as ex:
            return TruckerResponses.get_status_500(ex=ex)

    def descriptive_order_stats(self, previous_sum, current_sum, previous_start_date, previous_end_date, today_sum):
        """
        Descriptive Order report
        Provide data with respect to previous period and current period and current date fare summation
        """
        try:
            # ------------------ conditional response ------------------
    ...
    def trucker_fare_graph(self, result_data, time_zone, start_timestamp, end_timestamp, group_by, conversion_rate,
                        currency_symbol):
        try:
    ...
    def trucker_count_graph(self, result_data, time_zone, start_timestamp, end_timestamp, group_by):
        try:
            result_data["createdDate"] = pd.to_datetime(result_data["createdDate"], unit='s')
            result_data['createdDate'] = result_data['createdDate'].apply(
                lambda x: x.replace(tzinfo=UTC).astimezone(time_zone))
    ...
    def trucker_payment(self, order, time_zone, start_timestamp, end_timestamp, group_by, export):
        try:
    ...
    def trucker_status(self, order, time_zone, start_timestamp, end_timestamp, group_by, export):
        try:
            order = pd.DataFrame(order)
            order['bookingDateTimestamp'] = order['bookingDateTimestamp'].apply(
                lambda x: datetime.fromtimestamp(x, tz=time_zone))
    ...
    def top_trucker(self, order, sort, top, conversion_rate, currency_symbol):
        """
        ...
        try:
            ...
            zones = pd.DataFrame(db["operationZones_truckers"].find({"_id": {"$in": _ids}}, {"name": 1})).to_dict(
                orient="records")
            print("zones -----------_>", zones)
    ...
    def trucker_map_process(self, result_data, location):
        try:
    ...
    def trucker_count(self, order, time_zone, start_timestamp, end_timestamp, group_by, export):
        try:
            order = pd.DataFrame(order)
            order['bookingDateTimestamp'] = order['bookingDateTimestamp'].apply(
                lambda x: datetime.fromtimestamp(x, tz=time_zone))
    ...
    def trucker_fare(self, order, time_zone, start_timestamp, end_timestamp, group_by, export, currency,
